...
class DetectEnergy3d(Node):

    def __init__(self):
        super().__init__('detect_energy_3d')

        self.get_logger().info('Detecting in 3D')

        self.ball2d_sub  = self.create_subscription(Point,"/detected_energy",self.ball_rcv_callback, 10)
        self.ball3d_pub  = self.create_publisher(Point,"/detected_energy_3d",1)
        self.ball_marker_pub  = self.create_publisher(Marker,"/energy_3d_marker",1)
